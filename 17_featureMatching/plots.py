...
def plot_proc_time( config, epoch, chunk, proc_time_checkpoint):
    ...
    if(epoch>0 or chunk>0):
        ...
        proc_time_plot = np.zeros( (proc_time_checkpoint.shape[0], proc_time_checkpoint.shape[1]*proc_time_checkpoint.shape[2]) )
        
        for i in range(proc_time_checkpoint.shape[0]):
            proc_time_plot[i,:] = proc_time_checkpoint[i,:,:].reshape( (1, -1) )
            
        x_line = np.arange(0, proc_time_plot.shape[1], 1)
        
        lines = []
        for i in range(proc_time_plot.shape[0]):
            if(i==0):
                fig, ax0 = plt.subplots()
            elif(i==1):
                ax1 = ax0.twinx()
