...
def calculate_ess_loss_and_L2loss( config, inputs_model_B, xs_ess, R_device, t_device, virtPt_device ):
    ...
    A1 = torch_skew_symmetric(t_device)
    A2 = A1.reshape( (3,3) )
    A3 = torch.matmul(A2, R_device)
    e_gt_unnorm = torch.reshape( A3, (1,9) )
...
def weighted_8points(x_in, logits):
    ...
    logits = torch.squeeze(logits, axis=0)
...
def batch_episym(x1, x2, F):
    ...
    x1 = torch.cat([x1, x1.new_ones(batch_size, num_pts,1)], dim=2).reshape(batch_size, num_pts,3,1)
    x2 = torch.cat([x2, x2.new_ones(batch_size, num_pts,1)], dim=2).reshape(batch_size, num_pts,3,1)
    ...
    f1 = F.reshape(3,3)
    f2 = f1.unsqueeze(dim=0)
    f3 = f2.repeat(num_pts,1,1)
    F = f3.unsqueeze(dim=0)
